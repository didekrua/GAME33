...
public class MouseMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (rotationX > 35f)
        {
            rotationX = 35f; // 위로 35도 이상 넘어가지 못하게 (고개가 과하게 젖혀지지 않게)
        }

        if (rotationX < -30f)
        {
            rotationX = -30f; // 아래로 30도 이상 넘어가지 못하게 (고개가 과하게 꼬꾸라지지 않게)
        }

        transform.eulerAngles = new Vector3(-rotationX, rotationY, 0);

        
            if (Input.GetMouseButtonDown(0)) // 왼쪽 마우스 버튼을 클릭한 경우
            {
                ...
                if (Physics.Raycast(ray, out hit))
                {
                    ...
                    // 상호작용할 오브젝트에 대한 처리를 수행합니다.
                    // 예를 들어, 클릭한 오브젝트에 대한 동작을 수행하거나 특정 스크립트를 호출할 수 있습니다.
                }
            }
        }
